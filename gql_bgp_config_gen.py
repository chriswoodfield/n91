...
class Generator:
    ...
    async def build_model(self, device_name: str) -> list[NBModel.IPAddress]:
        ...
        lo_ip4_addr = str(IPNetwork(lo_addresses["lo_ip4"][0]).ip)
    ...
    async def get_vxlan_flood_ips(self, device_list: list[str]) -> str:
        """Returns the IPv4 loopbacks of given devices in a space-separate format."""
        ip_list = ""
        for device_name in device_list:
            ip_address_list = await self.get_int_ips(interface_name="Loopback0", 
                                            device_name = device_name, ip_version=4)
            # Only need the first address, strip the mask
            ip_list = ip_list + f"{str(IPNetwork(ip_address_list[0]).ip)} "
